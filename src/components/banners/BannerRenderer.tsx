 ...
 */
export function BannerZone({ placement, categoryId, q, fullWidth = false }: BannerZoneProps) {
...
function resolveLink(banner: Banner): string | null {
  switch (banner.link_type) {
    ...
    case "category":
      return banner.link_value ? `${paths.products}?category=${encodeURIComponent(banner.link_value)}` : null;
    case "brand":
      return banner.link_value ? `${paths.products}?brand=${encodeURIComponent(banner.link_value)}` : null;
    case "search":
      return banner.link_value ? `${paths.products}?q=${encodeURIComponent(banner.link_value)}` : null;
    case "external":
...
function ImageBanner({ banner, fullWidth }: { banner: Banner; fullWidth: boolean }) {
  return (
    <div className={`${fullWidth ? "w-full" : "mx-auto max-w-7xl"} h-72 overflow-hidden rounded-2xl border border-gray-200 bg-gray-100 shadow-sm dark:border-gray-800 dark:bg-gray-900`}>
...
function HeroCarousel({ banners, fullWidth }: { banners: Banner[]; fullWidth: boolean }) {
  ...
  return (
    <div className={`relative ${fullWidth ? "w-full" : "mx-auto max-w-7xl"} h-72 overflow-hidden rounded-2xl border border-gray-200 bg-gray-100 shadow-sm dark:border-gray-800 dark:bg-gray-900`}>
      {banners.map((b, i) => (
        <div
          key={b.uuid}
      ...
      {count > 1 && (
        <>
          <button
            type="button"
            onClick={() => go(-1)}
            className="absolute left-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow transition-colors hover:bg-white"
          ...
          <button
            type="button"
            onClick={() => go(1)}
            className="absolute right-3 top-1/2 flex h-9 w-9 -translate-y-1/2 items-center justify-center rounded-full bg-white/90 text-gray-700 shadow transition-colors hover:bg-white"
