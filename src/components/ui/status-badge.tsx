...
type VariantType = 'default' | 'success' | 'warning' | 'danger' | 'info' | 'secondary'

const statusConfig: Record<ValidStatus, { label: string; variant: VariantType }> = {
  ...
  return (
    ...
    >
      <span className="relative flex h-2 w-2">
        ...
        />
        <span
          className={cn('relative inline-flex h-2 w-2 rounded-full', getDotColor(config.variant))}
        />
      </span>
      {config.label}
    </div>
...
export { getStatusBadgeConfig, getStatusBadgeVariantClasses, StatusBadge }
