  ...
  return (
    ...
    >
      <div className="relative">
        ...
        {/* Zoom controls */}
        {current && (
          <div className="absolute right-3 bottom-3 flex items-center gap-1 rounded-full bg-black/60 p-1 text-white">
            <button
              type="button"
              onClick={zoomOut}
            ...
            <button
              type="button"
              onClick={resetZoom}
              className="min-w-10 rounded-full px-2 py-1 font-medium text-xs transition-colors hover:bg-white/20"
            >
              {scale}x
            </button>
            <button
      ...
      {/* Thumbnails */}
      {images.length > 1 && (
        <div className="flex flex-wrap justify-center gap-2 border-gray-200 border-t p-3 dark:border-gray-700">
          {images.map((img, i) => (
            <button
              key={img}
              type="button"
