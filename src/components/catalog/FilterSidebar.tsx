...
export function FilterSidebar({
  ...
  const visibleBrands = showAllBrands ? filteredBrands : filteredBrands.slice(0, FILTER_PREVIEW_COUNT);
  const visibleFeatures = showAllFeatures ? filteredFeatures : filteredFeatures.slice(0, FILTER_PREVIEW_COUNT);
  ...
  return (
    <aside className="space-y-6">
      ...
      {/* Brands */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Brands</h3>
        ...
        <div className="space-y-1">
          {visibleBrands.map((brand) => (
            <div key={brand.id} className="flex items-center gap-2">
              ...
              />
              <label htmlFor={`brand-${brand.id}`} className="cursor-pointer text-sm text-gray-700 dark:text-gray-300">
                {brand.name}
              </label>
            </div>
          ))}
        ...
        {filteredBrands.length > FILTER_PREVIEW_COUNT && (
          ...
          >
            ...
            <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${showAllBrands ? "rotate-180" : ""}`} />
          </button>
        )}
      </div>
      ...
      {/* Price */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Price range</h3>
        <div className="space-y-1">
      ...
      {/* Features */}
      <div>
        <h3 className="mb-2 text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Features</h3>
        <div className="relative mb-2">
          <Search className="absolute left-2.5 top-1/2 h-3.5 w-3.5 -translate-y-1/2 text-gray-400" />
          <Input
            value={featureSearch}
          ...
          {visibleFeatures.map((feature) => (
            <div key={feature.id} className="flex items-center gap-2">
              <input
                ...
                onChange={() => onChange({ featureIds: toggleId(selection.featureIds, feature.id) })}
                className="h-4 w-4 rounded border-gray-300 text-blue-600 focus:ring-blue-500"
              />
              <label htmlFor={`feature-${feature.id}`} className="cursor-pointer text-sm text-gray-700 dark:text-gray-300">
                {feature.name}
              </label>
            </div>
          ))}
        ...
        {filteredFeatures.length > FILTER_PREVIEW_COUNT && (
          ...
          >
            ...
            <ChevronDown className={`h-3.5 w-3.5 transition-transform duration-200 ${showAllFeatures ? "rotate-180" : ""}`} />
          </button>
        )}
      </div>
    </aside>
