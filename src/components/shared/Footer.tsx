...
export function Footer() {
  ...
  return (
    <footer className="border-t border-gray-200 bg-white text-gray-600 dark:border-gray-800 dark:bg-gray-950 dark:text-gray-300">
      {/* Newsletter band */}
      <div className="border-b border-gray-200 bg-gradient-to-r from-blue-50 via-white to-cyan-50 dark:border-gray-800/80 dark:from-blue-950/50 dark:via-gray-950 dark:to-cyan-950/40">
        <div className="mx-auto flex max-w-[1920px] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          <div>
            <h2 className="text-xl font-bold text-gray-900 sm:text-2xl dark:text-white">Stay ahead of the market</h2>
          ...
          {subscribed ? (
            <div className="flex items-center gap-2 rounded-xl border border-green-200 bg-green-50 px-5 py-3 text-sm font-medium text-green-700 dark:border-green-500/30 dark:bg-green-500/10 dark:text-green-400">
            ...
            <form onSubmit={handleSubscribe} className="flex w-full max-w-md gap-2">
              ...
              <input
                ...
                className="h-11 flex-1 rounded-xl border border-gray-300 bg-white px-4 text-sm text-gray-900 placeholder:text-gray-400 focus:border-blue-500 focus:outline-none focus:ring-2 focus:ring-blue-500/30 dark:border-gray-700 dark:bg-gray-900 dark:text-white dark:placeholder:text-gray-500"
              />
              <button
                type="submit"
                className="inline-flex h-11 items-center gap-2 rounded-xl bg-blue-600 px-5 text-sm font-semibold text-white transition-colors hover:bg-blue-700"
              >
        ...
        <div className="grid gap-10 lg:grid-cols-12">
          ...
          <div className="lg:col-span-4">
            ...
            <p className="mt-4 max-w-md text-sm leading-relaxed text-gray-500 dark:text-gray-400">
              HTAShop is an e-commerce platform that connects buyers with verified
              suppliers — shop online with secure payments, fast delivery, and
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              {TRUST_ITEMS.map((item) => (
                <div key={item.title} className="flex items-start gap-2.5 rounded-xl border border-gray-200 bg-gray-50 p-3 dark:border-gray-800 dark:bg-gray-900/60">
          ...
          <div className="lg:col-span-2">
            ...
            <ul className="mt-4 space-y-3">
              {CONTACT.map((item) => (
                <li key={item.label} className="flex items-start gap-2.5 text-sm text-gray-500 dark:text-gray-400">
                  <item.icon className="mt-0.5 h-4 w-4 shrink-0 text-gray-400 dark:text-gray-500" />
                  {item.href ? (
                    <a href={item.href} className="transition-colors hover:text-blue-600 dark:hover:text-white">
                      {item.label}
                    </a>
                  ) : (
      ...
      <div className="border-t border-gray-200 dark:border-gray-800/80">
        <div className="mx-auto flex max-w-[1920px] flex-col gap-2 px-4 py-6 sm:px-6 lg:flex-row lg:items-center lg:justify-between lg:px-8">
          ...
          <p className="text-xs text-gray-500">Secure online shopping · Fast global delivery</p>
        </div>
      </div>
    </footer>
  );
