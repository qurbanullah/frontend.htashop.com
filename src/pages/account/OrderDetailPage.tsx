    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          <span
            className={`inline-flex items-center rounded-full px-3 py-1 font-medium text-xs ${STATUS_STYLES[order.status] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}
          >
        ...
        <div className="rounded-2xl border border-gray-200 bg-white p-6 dark:border-gray-800 dark:bg-gray-900">
          <div className="flex items-center">
            {TIMELINE.map((step, index) => (
              <div
                key={step}
                className={`flex items-center ${index < TIMELINE.length - 1 ? 'flex-1' : ''}`}
              >
                ...
                {index < TIMELINE.length - 1 && (
                  <div
                    className={`mx-2 mb-5 h-0.5 flex-1 ${index < statusIndex ? 'bg-green-500' : 'bg-gray-200 dark:bg-gray-700'}`}
                  />
                )}
              </div>
            ))}
      ...
      {isTerminal && (
        <div
          className={`rounded-2xl border p-5 font-medium text-sm ${
                ...
                <div key={item.uuid} className="flex items-center gap-4 px-5 py-4">
                  {item.image_url ? (
                    <img
                      src={item.image_url}
                      alt={item.name}
                      className="h-16 w-16 shrink-0 rounded-xl border border-gray-200 object-cover dark:border-gray-700"
                    />
                  ) : (
                  ...
                  <div className="text-right">
                    ...
                    {item.base_price != null &&
                      Number(item.base_price) > Number(item.unit_price) && (
