...
function OrderStatusBadge({ status }: { status: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center rounded-full px-2.5 py-0.5 font-medium text-xs ${styles[status] ?? 'bg-gray-100 text-gray-700 dark:bg-gray-800 dark:text-gray-300'}`}
    >
      {label}
    </span>
  )
    ...
    <div className="space-y-8">
      ...
      <div>
        <div className="mb-4 flex items-center justify-between">
          <h2 className="font-semibold text-gray-900 text-lg dark:text-white">Recent orders</h2>
          <Link
